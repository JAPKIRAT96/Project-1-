...
function KtoF(value) {
  // kelvin to foreighheight conversion
...
function getFromLocalStorage() {
  ...
  if (value) {
    ...
    value.map((item) => {
      $("#history-ul").append(
        `<li class="list-group-item" id="history-item">${item}</li>`
      );
    });
  } else {
    $("#histryData").hide();
...
async function getFivedayForcast(cityName) {
  ...
  await fetch(Url)
    .then((resp) => resp.json())
    .then((data) => {
      const selectedData = data.list.filter((item, i) =>
        possibleValue.includes(i)
      );
              ...
              <div class="col forcastCard">
                <div class="card card-col">
                  <div class="card-body text-white">
                     <h2 style="font-size: 1.8rem;">${getSlicedDate(
                       item.dt_txt
                     )}</h2>
                     <img height="70" width="70" src="http://openweathermap.org/img/wn/${
                       item.weather[0].icon
                     }@2x.png" alt="icon">
                     <p>Temp: ${KtoF(item.main.temp)
                       .toFixed(3)
                       .replace(/\.(\d\d)\d?$/, ".$1")}°F</p>
  ...
  fetch(url)
    ...
    .then((data) => {
      $("#cityName").append(
        `<h1 id="cityName">${
          data.name
        } ${DateNow()} <img src="http://openweathermap.org/img/wn/${
          data.weather[0].icon
        }@2x.png" alt="icon"></h1>
            <p>Temperature: ${KtoF(data.main.temp)
              .toFixed(3)
              .replace(/\.(\d\d)\d?$/, ".$1")} °F</p>
...
async function getUvIndex(lat, lon) {
  ...
  await fetch(uvUrl)
    ...
    .then((data) => {
      $("#cityName").append(
        `<div class="d-flex"><div>UV Index: </div><div class="ml-1 uv">${data.value}</div></div> `
      );
    });
}
